...
import pygame, os, time, datetime, copy, string,random, sys, moviepy, imageio
...
class InfantAttentionApp(object):
    ...
    def init(self, screen = "home"):
        ...
        self.currentVideo = 0 #####
        self.videoList = self.initVideoList()
        self.videoBar = [] #####
        self.videoButtons = [] #####
        #self.videoBoolean = False ######
        self.videoListLength = len(self.videoList)
        #Saftey Check for User Input
        self.checkUserVars()
        self.initTrialRecord()

    def initVideoList(self):
        result = os.listdir("Videos/")
        if '.DS_Store' in result: result.pop(0)
        return result
    ...
    def getSamplePool(self):
        rounds = self.imgsPerTrial // self.mainPoolSize
        remainder = self.imgsPerTrial % self.mainPoolSize
...
############################### HOME SCREEN ####################################
    ...
    def updateColor(self):
        ...
        for button in self.videoButtons: button.updateColor(self.invertBgCol())
        for button in self.videoBar: button.updateColor(self.invertBgCol())
        ...
        for button in self.homeButtons:
            if button.isClicked(x,y):
                if button.msg in ["Animations","Reset"] and self.animationInput:
                    ...
                    if self.videoListLength == 0:                         
                        self.screen = "trial"
                        self.getSamplePool()
                        self.applyAnimations()
                        self.nextTrial()
                    else: self.watchVideo()
        for button in self.videoButtons:
            if button.isClicked(x,y):
                if button.msg == "L":
                    self.currentVideo = (self.currentVideo + 1) % self.videoListLength
                elif button.msg == "R":
                    self.currentVideo = (self.currentVideo + 1) % self.videoListLength

                self.videoBar[0].textUpdate( self.videoList[self.currentVideo] )
    ...
    def homeTimerFired(self, dt):
        ...
        if self.videoButtons == [] and self.videoListLength > 0: #####
            self.videoButtons.append(Button(8*self.width//10-119,6*self.height//7,40,40,"L","blue","blue",30))
            self.videoButtons.append(Button(8*self.width//10+119,6*self.height//7,40,40,"R","blue","blue",30))
        if self.videoBar == [] and self.videoListLength > 0:#####
            self.videoBar.append(Button(8*self.width//10,6*self.height//7, 200,40,self.videoList[self.currentVideo],"blue","blue",15))
        ...
        if self.requestAnimations and self.imgsToAnimate != []:
            ...
            self.animationButtons.append(Button(17.4*self.width//24,4*self.height//10,150,30,self.animationMessage,"blue","blue"))   
            self.requestAnimations = False

        errorCount = 0
        for intBox in self.homeIntInputs:
            if (intBox.invalidData or intBox.quantData == None) and not (intBox.label.startswith("Images to A")): 
                errorCount += 1
    ...
    def homeDrawAll(self,screen):
        ...
        for button in self.videoButtons:
            button.draw(screen)
        for bar in self.videoBar:
            bar.draw(screen)

############################### VIDEO SCREEN ###################################
    def watchVideo(self):
        path = "Videos/" + self.videoList[self.currentVideo]
...
################################################################################
    def __init__(self, width=1200, height=800, fps=300, title="Infant Attention App"):
        self.width = width
        self.height = height
        self.fps = fps
        self.title = title
