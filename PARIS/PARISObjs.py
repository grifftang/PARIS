...
class Button(object):
    def __init__(self, centerX, centerY, width, height, text, color, textColor):
    ...
    def checkData(self):
        if (self.tag == "seconds"):
            ...
            for char in list(self.data):
                if char not in self.nums: self.invalidData = True
                else: count += 1
            ...
            count = 0
            for char in list(self.data):
                if ((char not in (self.nums)) and (char not in self.punct)) or char == ".": self.invalidData = True
                else: count += 1
            if count == len(list(self.data)): self.invalidData = False
            self.quantizeList()
            ...
            count = 0
            for char in list(self.data):
                if char not in self.nums or char == ".": self.invalidData = True
                else: count += 1
            if count == len(list(self.data)): self.invalidData = False
            self.quantizeNum()
...
class Selector(object):
    def __init__(self, centerX, centerY,path,width,height,color):
        self.centerX = centerX
        self.centerY = centerY
        self.folderPath = path
        self.width,self.height = width,height
        self.colorList = self.initColors
        ...
        for path, dirs, files in os.walk("Trial Record"):
            for file in files:
                if file == '.DS_Store': pass
                    ...
                    options.append((text,os.path.join(path,file)))
        self.options = options
        if not (len(self.options) == 0): self.quantData = self.options[0]
        else: self.options.append(self.font.render("No Videos In Folder", True, self.color))

    def draw(self,screen):
        centerRect = (self.centerX-self.quantData[0].get_width//2,self.centerY-self.height//2,self.quantData[0].get_width,self.height)
        pygame.draw.rect(screen,self.color,centerRect,1)
        screen.blit(self.quantData[0],(self.centerX - self.quantData[0].get_width//2,self.centerY - self.quantData[0].get_height()//2))
"""
