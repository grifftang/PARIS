...
import pygame, moviepy, os, time, datetime, copy, string, imageio
...
class InfantAttentionApp(object):
        ...
        for path, dirs, files in os.walk("MainPool"):
            for file in files:
                if file == '.DS_Store': pass
                    ...
                    self.mainPool.add(Stimulus(file,os.path.join(path,file),self.width))
    ...
    def getSamplePool(self):
        added = set()
        for i in range(self.imgsPerTrial):
            stimulus = self.mainPool.pop()
            self.testPool.append(copy.copy(stimulus))
            added.add(stimulus)
        for obj in added: self.mainPool.add(obj)
...
############################### VIDEO SCREEN ###################################
    videos = set()
    selectedVideo = tuple()
    def videoTimerFired(self, dt):
        if self.videos == set():
            count = 0
            for path, dirs, files in os.walk("Videos"):
                for file in files:
                    if file == '.DS_Store': pass
                    else:
                        print(os.path.join(path,file))
                        self.videos += set(VideoFileClip(os.path.join(path,file)))
                        count += 1
            if count == 0: 
                self.screen = "trial"
                self.getSamplePool()
                ...
                self.selectedVideo = self.videos.pop()
                self.videos += set(self.selectedVideo)
                self.selectedVideo.preview()
    ...
    def videoDrawAll(self, screen):
        #if self.selectedVideo != tuple():
        #    screen.blit(self.selectedVideo[1],(0,0))
        pass
...
############################### HOME SCREEN ####################################
    ...
    def homeTimerFired(self, dt):
        ...
        if self.homeIntInputs == []:
            ...
            self.homeIntInputs.append(InputBox(3.5*self.width//10,2*self.height//10,"Images Per Trial:","of %d in Library"%(self.mainPoolSize),"blue"))
